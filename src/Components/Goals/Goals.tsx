...
const Goals = ({ goals, setGoals, setShowGoalsSettings, setReward, goalAmount, setGoalAmount, showGoalSettings, setStreak, streak }: Props) => {
  ...
  return (
    <>{showGoalSettings ? <GoalsSettings setShowGoalsSettings={setShowGoalsSettings} goalAmount={goalAmount} setGoalAmount={setGoalAmount} /> : <div className="flex p-4 flex-col items-center">
    ...
    />
    <button className="border-2 shadow-md shadow-black border-black hover:-translate-y-0.5 active:translate-y-0 active:bg-[#4f5f98] rounded-lg px-4 active:shadow-inner hover:bg-[#7289da] active:border-2" onClick={createGoal}>Create goal</button>
    </>
    }
    {!!goals.length &&
      goals
        .map((goal, i) => (
          <div className="w-full m-2">
            <Goal name={goal.name} id={goal.id} description={goal.description} done={goal.done} />
            <button className="border-2 shadow-md shadow-black border-black hover:-translate-y-0.5 active:translate-y-0 active:bg-[#4f5f98] rounded-lg px-4 active:shadow-inner hover:bg-[#7289da] active:border-2" onClick={() => removeGoal(i)}>🔴</button>
            <button className="border-2 shadow-md shadow-black border-black hover:-translate-y-0.5 active:translate-y-0 active:bg-[#4f5f98] rounded-lg px-4 active:shadow-inner hover:bg-[#7289da] active:border-2" onClick={() => completeGoal(i)}>🟢</button>
          </div>
        ))
        .slice(0, goalAmount)}
